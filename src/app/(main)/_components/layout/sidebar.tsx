...
import { useFollowings } from '@/hooks/api/stream';
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <nav className="flex-1">
        ...
        {/* 팔로잉 중인 채널 */}
        {!followings.isLoading && followings.data != null && (
          <div className="px-2 pt-4">
              ...
              {followings.data.map((following) => (
                <li key={following.id}>
                  <Link href={`/channels/${following.stream.streamer.nickname}`}
                    ...
                    <div>
                      <div className="font-medium">{following.stream.streamer.nickname}</div>
                      {following.stream.category != null && (<div className="text-xs text-muted-foreground">{following.stream.category.name}</div>)}
                    </div>
                  </Link>
                </li>
              ))}
